...
class Decoder_base(nn.Module):         
    def __init__(self, channel_0=32, channel_1=32, channel_2=64, channel_3=128, n_class=1):
        """
        Initialize the Primary Decoder based on the 4-layer primary feature architecture (adapted for StarNet).
        channel_0: Number of channels for m0 (32)
        channel_1: Number of channels for m1 (32)
        channel_2: Number of channels for m2 (64)
        channel_3: Number of channels for m3 (128) - Deepest feature layer F4
        """
...
if __name__=='__main__':
    # Simulate the output features of a 4-layer encoder (Batch=2), adapted for StarNet dimensions
    m0 = torch.rand(2, 32, 128, 128)
    m1 = torch.rand(2, 32, 64, 64)
    m2 = torch.rand(2, 64, 32, 32)
    m3 = torch.rand(2, 128, 16, 16)
  
    # Instantiate the Primary Decoder (passing the corresponding channel dimensions)
    decoder_net = Decoder_base(channel_0=32, channel_1=32, channel_2=64, channel_3=128, n_class=1)  
    
    # Forward pass
    pre, last_feat = decoder_net(m0, m1, m2, m3)

    print("Final Prediction Mask Shape:", pre.size())        # Expected: [2, 1, 256, 256]
    print("Low-level Feature Shape for Subsequent Branches (Last Feature):", last_feat.size()) # Expected: [2, 32, 128, 128]